...
1.  Have a `@Validation.Validated` annotated type and use a
    `TypeValidator` service to validate it

2.  Invoke the constraint checks directly using `Validators` static
    methods

The feature fit with our [Helidon
Declarative](../se/injection/declarative.xml#Dec-Validation),
...
The following annotation processing setup must be done to generate the
code:
...
An object can be validated using one of the built-in constraints through
methods on
...
constraints) through programmatic API. Note that an instance of the
validator can be cached as long as it is used for the same type and
