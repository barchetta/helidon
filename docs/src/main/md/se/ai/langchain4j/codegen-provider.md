...
all of them in Helidon is very challenging, and it may happen that the
provider you are looking for is not one of those we are providing out of
the box. But you can generate integration yourself the same way we are
...
for, here we are using Google AI Gemini LangChain4j provider as an
example:
...
You also need to extend the list of annotation processors with
**builder** codegen:
...
generation, it can be package private. Name of the provider interface
needs to comply with convention and end with `Lc4jProvider`, prefix is
...
- Name of the provider needs comply with convention and end with
  `Lc4jProvider`, prefix is used for deriving a config key

- Some properties can be too complex for configuration, we can supply
  them via injection instead

You may notice that the **safetySettings** property is manually
configured in the provider interface, you can do that for the properties
that are too complex for setting via configuration. By adding annotation
...
property has the type `java.util.List` all the beans of its generic type
are injected.
...
It is possible to configure named qualifiers for injected beans, config
property `service-registry.named` prefixed with the key of desired
property is used as named qualifier for lookup when such property
exists.
...
- `dev.langchain4j.model.chat.request.ChatRequestParameters` default
  parameters for all the models

- `dev.langchain4j.http.client.HttpClientBuilder` Custom http client

- `dev.langchain4j.model.chat.listener.ChatModelListener` Chat model
