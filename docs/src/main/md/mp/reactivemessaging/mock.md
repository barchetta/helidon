...
5](/../../testing/testing.adoc)
or
[TestNG](/../../testing/testing-ng.adoc).
...
- Inject mock connector so we can access publishers and subscribers
  registered within the mock connector

- Messaging processing method connecting together channels
  `test-channel-in` and `test-channel-out`

- Actual JUnit 5 test method which is going to block the thread until 3
  items are intercepted on `test-channel-out` channel’s downstream and
  assert those with expected values.
