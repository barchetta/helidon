...
as metrics and health—​keep their familiar endpoints. The endpoints for
...
a dependency on Helidon’s MP metrics or health component, for example,
brings along the necessary observability components automatically.

To include other observability features in your Helidon MP application,
add one or more of the following dependencies.
...
[configuration](#_configuration) section below for more information.
...
Configuration in `META-INF/microprofile-config.properties` Assigning
Feature Weight to Control Routing
...
Some observer endpoints—​metrics and health—​were present in earlier
releases of Helidon MP. By default those continue to use their customary
paths (`/metrics`, `/health`). You can customize the endpoint for each
of those observers are described in the documentation for each observer.

Other observers have no counterpart in the MicroProfile spec and they
respond by default at subpaths of `/observe` as described below.
...
1.  For MicroProfile technologies (metrics, health) refer to the Helidon
    ...
    - [metrics
      config](../mp/metrics/metrics.xml#config-intro)
      documentation

    - [health
      config](../mp/health.xml#_configuration)
      documentation
